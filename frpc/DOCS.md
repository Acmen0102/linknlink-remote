...
> **提示**
> 
> 如果您还没有 LinknLink 账号，请联系管理员申请开通。
...
2. **配置文件**: 保存在 `/config/device_id.txt`

> **重要**
> 
> 请妥善保管您的设备 ID，在需要技术支持时可能会用到。
...
> **常见错误**
> 
> - **账号不存在（status: -46009）**: 表示该邮箱未在平台注册，请联系管理员添加账号
...
1. 检查网络连接是否正常
2. 确认可以访问 `euhome.linklinkiot.com` 和 `euadmin.linklinkiot.com`
3. 查看日志中的详细错误信息
4. 尝试重启 Add-on
...
1. 确认登录成功（检查日志中的 Company ID 和 User ID）
2. 检查设备 ID 是否正常生成
3. 查看完整的错误响应信息
4. 联系技术支持
...
> **警告**
> 
> 不要手动编辑 `/config/frpc.toml` 文件。该文件由 Add-on 自动生成和管理，手动修改可能导致连接失败。

如果配置文件损坏或丢失：

1. 重启 Add-on 会自动重新生成配置
2. 设备 ID 会保持不变（从 `/config/device_id.txt` 读取）
...
- **配置文件位置**: `/config/frpc.toml`
- **设备 ID 文件**: `/config/device_id.txt`

这些文件在 Add-on 每次启动时会自动生成或更新，无需手动备份。

> **提示**
> 
> 如果您需要迁移到新的 Home Assistant 实例，只需在新实例中使用相同的账号密码配置即可。设备 ID 会根据新设备的 MAC 地址自动生成。
